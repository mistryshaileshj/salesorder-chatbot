...
#   2+ dimensions -> a Sub-total row after each value of the FIRST dimension,
#                    then a Grand Total row.
...
def _build_totals(df: pd.DataFrame, dims: list, measures: list):
    """Return (augmented_df, is_total_mask).

    Detail rows are preserved in their original order. For 2+ dimensions the
    rows are grouped by the first dimension (biggest group first) so each
    sub-total sits directly under its block, even when the SQL ordered rows by
    the measure and interleaved the groups."""
    d = df.copy()
    for c in dims:
    ...
    if len(dims) == 1:
        ...
        first = dims[0]
        if any(_is_additive(m) for m in measures):
            om = next(m for m in measures if _is_additive(m))
            group_order = (d.groupby(first)[om].sum()
                             .sort_values(ascending=False).index.tolist())
        else:                                # no additive measure -> keep first-seen order
            group_order = list(pd.unique(d[first]))
        for g in group_order:
            block = d[d[first] == g]
            emit_detail(block)
            sub = {c: "" for c in d.columns}
            sub[first] = f"{g} \u2014 Total"           # e.g. "Nike — Total"
            sub.update(_sum_measures(block, measures))
    ...
    rows.append(grand); is_total.append(True)
...
def _show_table_with_totals(df: pd.DataFrame, dims: list, measures: list, key=None):
    ...
    aug, is_total = _build_totals(df, dims, measures)
    disp = aug.rename(columns={c: _pretty(c) for c in aug.columns})
    # Sr. No: number the detail rows only; total rows get a blank.
    srno, n = [], 0
    for t in is_total:
        if t:
            srno.append("")
    ...
    first_dim_label = _pretty(dims[0])

    def _highlight(row):
        if is_total[row.name]:
            is_grand = str(row[first_dim_label]).strip() == "Grand Total"
            bg = "#d7deea" if is_grand else "#eef1f5"          # grand a touch darker
